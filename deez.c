...
void testSpan(char *input) {
    printf(">>> %s\n", input);

    // split the input with spaces
    CharSpan args[4];
    int found = split(args, 4, input, " ");

    // handle the input
    CharSpan *arg0 = &args[0];
    if(compareSpan(arg0, "help")) {
        printf("Help page!\n");
    }
    else if(compareSpan(arg0, "exit")) {
        printf("Exiting!\n");
    }
    else if(compareSpan(arg0, "move")) {
        if(found >= 3) {
            CharSpan *arg1 = &args[1];
            int num = spanToInt(args[2]);
            if(compareSpan(arg1, "x")) {
                printf("Moved %d in x-direction\n", num);
            }
            else if(compareSpan(arg1, "y")){ 
                printf("Moved %d in y-direction\n", num);
            } else {
                printf("Invalid direction\n");
            }
        }
        else {
            printf("Invalid args count: move <direction> <amount>\n");
        }
    } else {
